"""
1.ACM 호텔 매니저 지우는 손님이 도착하는 대로 빈 방을 배정하고 있다.
...
for i in range(0,Num):
    ...
    for j in range(1,W+1):
        for k in range(1,H+1):
            if j<10:
                room_list.append(str(k)+"0"+str(j))
            else:
                room_list.append(str(k)+str(j))
    print(room_list[N-1])
    del room_list[:]

#나는 손님이 예약하면 그 다음 방을 다른 손님에게 계속 안내해야되는 더 어려운 문제인줄 알고 막 고민하다가 읽어보니까 그게 아니었넹..
#그게 더 재밌을거 같은데 내일 이걸 그렇게 한 번 업그레이드 시켜봐야겠다
#매번 느끼지만 del로 초기화시키는거 진짜 꿀이다
